...
describe("classifyReply", () => {
  ...
  it("pede ao modelo o prazo de retomada indicado pelo próprio lead", async () => {
    const parse = vi.fn().mockResolvedValue({
      parsed_output: classificacao({
        intent: "not_now",
        suggested_resume_days: 14,
      }),
      stop_reason: "end_turn",
    });

    const resultado = await classifyReply(
      "Me procure em duas semanas.",
      depsComParse(parse),
    );

    expect(resultado.suggested_resume_days).toBe(14);
    expect(parse.mock.calls[0]![0].system[0].text).toContain(
      "suggested_resume_days",
    );
  });
